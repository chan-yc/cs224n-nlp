...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        W = torch.empty(self.embed_size*self.n_features, self.hidden_size)
        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(W))

        b1 = torch.empty(self.hidden_size)
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(b1))

        self.dropout = nn.Dropout(self.dropout_prob)

        U = torch.empty(self.hidden_size, self.n_classes)
        self.hidden_to_logits_weight = nn.Parameter(nn.init.xavier_uniform_(U))

        b2 = torch.empty(self.n_classes)
        self.hidden_to_logits_bias = nn.Parameter(nn.init.uniform_(b2))
        ### END YOUR CODE
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.tensor([self.embeddings[indices].view(-1).detach().numpy() for indices in w])
        ### END YOUR CODE
        return x
    ...
    def forward(self, w):
        ...
        h_d = self.dropout(h)
        logits = h_d @ self.hidden_to_logits_weight + self.hidden_to_logits_bias
        ### END YOUR CODE
        return logits
