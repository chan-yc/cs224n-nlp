...
def naiveSoftmaxLossAndGradient(
    ...
    gradCenterVec = np.inner(y_hat - y, outsideVectors.T)
...
def negSamplingLossAndGradient(
    ...
    # lamb is lambda defined in the written assignment question (g)(ii)
    lamb = outsideVectors[indices] @ centerWordVec
    lamb[1:] *= -1
    lamb = sigmoid(lamb)
    loss = -np.log(lamb).sum()
    lamb -= 1
    lamb[1:] *= -1
    gradCenterVec = lamb @ outsideVectors[indices]
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    for n, idx in enumerate(indices):
        gradOutsideVecs[idx] += lamb[n] * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        results = word2vecLossAndGradient(centerWordVec, outsideWordIdx,
                                          outsideVectors, dataset)
        loss += results[0]
        gradCenterVecs[centerWordIdx] += results[1]
        gradOutsideVectors += results[2]
